...
#---------ADDING NOISE
def augment_data(hand_data, obj_data):
    noise = torch.randn_like(hand_data) * 0.01  # Add small Gaussian noise
    augmented_hand_data = hand_data + noise
    return augmented_hand_data, obj_data
# Training loop
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        hand_pose_data, object_data = augment_data(hand_pose_data, object_data)
        optimizer.zero_grad()
...
# Save the model
torch.save(model.state_dict(), '/cvae_03Noise_weights.pth')
...
with open('/cvae_03Noise_losses_errors.pkl', 'wb') as f:
